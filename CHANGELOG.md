...
- 修复：远程 NVIDIA 显卡仅在匹配成功时使用 nvidia-smi 补齐，避免把 AMD 显卡误识别为 NVIDIA
- 修复：远程数值解析支持 0%（不再显示为“暂无”）

## v0.3.7

- 改进：NVIDIA 显卡占用/显存/温度优先使用 nvidia-smi（AIDA64 仅用于热点温度覆盖）
- 修复：多显卡场景下 AMD 条目丢失与重复 NVIDIA 条目问题

## v0.3.8

- 修复：远程输出中出现占位显卡（如 GPU2）
